...
await tap.test('Sequelize Integration (Modern API)', async (t) => {
	...
	t.test(async (t) => {
		...
		t.test('supports several concurrent transactions', async (t) => {
			...
			await Promise.all([
				sequelize.transaction(async () => {
					t1id = als.get('transaction').id;
				}),
				sequelize.transaction(async () => {
					t2id = als.get('transaction').id;
				}),
			]);
			t.ok(t1id);
			t.ok(t2id);
		...
		t.test('supports nested promise chains', async (t) => {
			await sequelize.transaction(async () => {
				const tid = als.get('transaction').id;

				await User.findAll();
				t.ok(als.get('transaction').id);
				t.equal(als.get('transaction').id, tid);
			});
			t.end();
		});
	...
	t.test(async (t) => {
		...
		t.test('promises returned by sequelize.query are correctly patched', async (t) => {
			await sequelize.transaction(async (transaction) => {
				...
				t.equal(als.get('transaction'), transaction);
			});
		});
		...
		t.test('custom logging with benchmarking has correct CLS context', async (t) => {
			const logger = t.captureFn((...rgs) => {
				rgs.slice();
				return als.get('value') as number;
			});
